#!/usr/bin/env bash
# =============================================================================
#  run_baseline_multi_sdn.sh — Run ALL CDN Baseline experiments WITH Ryu SDN
...
echo "============================================================"
echo "  CDN Baseline SDN Experiment Runner"
...
for ROUND in "${ROUND_LIST[@]}"; do
    ...
    if sudo bash "$PROJECT/CDN_baseline/run_baseline_sdn.sh" \
           --sit   "$SIT"   \
           --speed "$SPEED" \
           --round "$ROUND"; then
        echo "  [OK] $RUN_ID"
        ...
        echo "  [FAIL] $RUN_ID"
        FAILED=$(( FAILED + 1 ))
        echo "$RUN_ID" >> "$PROJECT/results/cdn_baseline_sdn_failed.txt"
    fi
    sleep 5
...
echo "  Results: $PROJECT/results/cdn_baseline/sdn/"
echo "============================================================"