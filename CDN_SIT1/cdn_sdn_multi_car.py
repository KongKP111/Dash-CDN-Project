...
# Explicit elapsed-time cutoff for the per-tick handover state machine below
# (one association attempt per tick, re-targeted fresh each attempt) --
# matches Situation1_DASH/platoon_topology.py's own handover_timeout=8.0
# default exactly, so both arms give up on a struggling handover at the
# same real-time point instead of CDN's old implicit ~8.1s-from-retry-count
# approximation (6 retries@1.0s + 0.3s + 3 retries@0.6s).
...
def run_platoon(cars, server, rsu_objs, throttle, vlc_paths_list, out_dir,
    ...
    # cum_outage_s: cumulative REAL seconds each car has spent with zero
    # association (all retries exhausted, ensure_assoc() never returned
    # "Connected to"). Ported from CDN_SIT2/cdn_sdn_hight_speed.py's outage
    # tracking (see DASH_HANDOFF_SITUATION2.md).
    #
    # Now restructured into the SAME per-tick, one-attempt-per-tick state
    # machine as Situation1_DASH/platoon_topology.py's run_platoon() (which
    # itself already carries this design, ported from CDN_SIT2 -- see that
    # file's own comment). Previously this loop retried up to 9 times
    # back-to-back inside a single per-car iteration before giving up for
    # the tick, producing one chunky measurement sample per struggle instead
    # of one per attempt -- a real granularity mismatch against DASH's arm,
    # not just an order-of-magnitude coincidence. handover_timeout below is
    # now an explicit, monitored elapsed-time cutoff (HANDOVER_TIMEOUT_S),
    # matching platoon_topology.py's handover_timeout=8.0 exactly, rather
    # than an implicit ~8.1s approximated from a fixed retry-count budget.
    ...
    while True:
        ...
        for i, car in enumerate(cars):
            ...
            # either the zone changed, or the link spontaneously dropped
            # while still nominally on the same RSU. Same two-condition
            # check as CDN_SIT2/platoon_topology.py's run loops.
            if not handover_active[i]:
                needs_handover = (target_rsu != current_rsu[i])
                link_ok = True
                if not needs_handover:
                    link_ok = 'Connected to' in car.cmd(f'iw dev {car.name}-wlan0 link')
                if needs_handover or not link_ok:
                    ...
                    if needs_handover:
                        ...
                        # Pre-warm the target edge via the cooperative
                        # channel BEFORE the (slow, ~1-3s) wifi
                        # reassociation completes -- same "warm ahead of
                        # arrival" pattern as the single-vehicle SDN+CDN
                        # scenario. Harmless if several platoon-mates target
                        # the same RSU at once (idempotent).
            ...
            # One association attempt per tick -- NOT a blocking inner
            # retry loop. A handover that takes N attempts to resolve now
            # produces N real measurement samples (each with its own x/y,
            # RSSI, etc.) instead of one sample before and one after
            # everything finally resolves. Same shape as
            # Situation1_DASH/platoon_topology.py's run_platoon().
            if handover_active[i]:
                ...
                if ok:
                    ...
                    # every attempt starts with a real disconnect, so this
            ...
            if outage:
                # outage (interim retry OR final give-up) costs real
                # wall-clock time with zero connectivity -- charge THIS
                # tick's own real duration (dt) to cum_outage_s, same
                # convention as platoon_topology.py, so a struggle that
                # eventually succeeds still counts, not just ones that hit
                # the timeout and give up.
                cum_outage_s[i] += dt
                # Known, verified outage -- don't silently reuse the last
                # known-good RSSI/cache/latency as if nothing happened (the
                # bug this whole fix addresses): make the true "no link"
                # state explicit in every field instead.
                sig = last_signal[i]  # kept for the rssi_dbm column, but see 'outage' flag
                bw = 0.0
                ...
                # never got a X-Cache-Status header back. Cache HIT/MISS is
                # strictly an edge-content question -- a request that got no
                # answer at all is a connection/request LOSS, same bucket as
                # a verified outage, not a third cache tier.
                if cache == 'UNKNOWN':
                    cache = 'LOSS'
                loss_pct = loss_pollers[i].poll()
                vlc_stalling, vlc_buffer_pct, vlc_cum_stall_s = vlc_tel[i].poll()

            stall = int(latency >= 3.0 or cache == 'LOSS' or vlc_stalling or outage)
