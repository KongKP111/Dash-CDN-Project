...
SCRIPT_DIR="$PROJECT/CDN_SIT1_Test"
...
for CARS in "${CAR_COUNTS[@]}"; do
    ...
    # per-vehicle CSVs into CDN_SIT1_Test/result_multi_car/<run_id>/ itself;
    ...
    if compgen -G "$RESULT_DIR"/*_network.csv > /dev/null; then
        for csv in "$RESULT_DIR"/*_network.csv; do
            ...
            UNK_N=$(grep -c ",UNKNOWN," "$csv" 2>/dev/null || true)
            TOTAL=$(( HIT_N + MISS_N + UNK_N ))
            echo "     $car_name  →  HIT:${HIT_N}  MISS:${MISS_N}  UNKNOWN:${UNK_N}  (samples: ${TOTAL})"
        done
        echo "[OK] $RUN_ID"
    else
