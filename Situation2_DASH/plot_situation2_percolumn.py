#!/usr/bin/env python3
"""
plot_situation2_percolumn.py -- Situation 2 (Mobility Speed) SDN+DASH,
one column per speed. Mirrors Situation1_DASH/plot_situation1_percar.py's
exact 5-row-per-column visual template (RSU zone bands, dashed handover
lines, QoE annotation box) -- here "column" is speed (80/100/120 km/h)
instead of vehicle, since this scenario has one vehicle, not a platoon.

QoE row: real Yin et al. formula, same constants/shape as
CDN_baseline/dash_cdn_comparison.py's compute_dash_qoe() -- NOT
Situation1_DASH's segment-based version (that one needs segments.csv from
the custom Dash/client/dash_client.py; this arm uses real VLC instead, so
there's no segments.csv -- the per-network-tick version is the one that
actually matches this arm's own CSV shape). bitrate from the 'quality'
label, switch penalty vs the previous tick, and T_k (rebuffer term) is the
REAL elapsed dt for that tick when stall=1 -- not a flat 1.0-per-tick
assumption like the original dash_cdn_comparison.py uses, because
Situation 2's run_loop() derives variable per-tick dt from real wall-clock
time (see baseline_4rsu_topo.py's run_loop()), so a stalled tick's real
duration is already sitting right there in the data -- using it instead of
a fixed constant is a more literal reading of the same formula, not a
deviation from it.

Usage:
    python3 plot_situation2_percolumn.py --speeds 80 100 120
"""
import csv
import os
import sys
...
def compute_dash_qoe_tick(rows):
    """Per-tick Yin et al. QoE term, adapted from
    CDN_baseline/dash_cdn_comparison.py's compute_dash_qoe() for this arm's
    real wall-clock-variable dt (see module docstring)."""
    t = col(rows, 't')
...
def make_plot(speeds, out_path):
    ...
    for speed in speeds:
        ...
        if not rows:
            ...
            continue
        per_speed.append((speed, rows))
    if not per_speed:
        print('  [ERROR] nothing to plot')
        return

    fig, axes = plt.subplots(5, len(per_speed),
                              figsize=(4.6 * len(per_speed), 15.0),
                              facecolor='white', squeeze=False)
    fig.subplots_adjust(hspace=0.6, wspace=0.32, top=0.90)
    fig.suptitle(
        'Situation 2: Mobility Speed — SDN+DASH, 80/100/120 km/h '
        '(one column per speed)',
        fontsize=13, fontweight='bold', color='#1a1a1a', y=0.995)

    xmin_global = M4.START_X
    xmax_global = M4.END_X

    for col_i, (speed, rows) in enumerate(per_speed):
        x = col(rows, 'x')
        rssi = col(rows, 'rssi')
        bw = col(rows, 'bw_mbps')
        loss = col(rows, 'loss')
        ...
        col_title = f'{speed} km/h  (n={len(rows)})'
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(
        description='Situation 2 (SDN+DASH), one column per speed -- mirrors plot_situation1_percar.py')
    p.add_argument('--speeds', type=int, nargs='+', default=[80, 100, 120])
    p.add_argument('--out', type=str,
                    default=os.path.join(_HERE, 'plots', 'percolumn_speed_comparison.png'))
    args = p.parse_args()
